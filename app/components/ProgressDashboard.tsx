  ...
  return (
    <div className="bg-white/90 rounded-2xl p-6 shadow-lg">
      ...
      <div className="text-center mb-4">
        ...
        <h2 className="text-2xl font-bold text-orange-600">{playerName}</h2>
      ...
      {showAvatarSelector && (
        <div className="mb-4 p-3 bg-purple-50 rounded-xl">
          <p className="text-sm font-bold mb-2">Pilih Avatar:</p>
          <div className="flex flex-wrap gap-2 justify-center">
            {avatars.map(avatar => (
              <button
                key={avatar.id}
                onClick={() => changeAvatar(avatar.emoji)}
                className={`text-3xl p-2 rounded-full transition ${avatar.color} ${
                  selectedAvatar === avatar.emoji ? 'ring-4 ring-purple-400 scale-110' : ''
                }`}
              >
                {avatar.emoji}
              </button>
      ...
      {/* Daily Streak Card */}
      <div className={`rounded-xl p-4 mb-4 text-center ${dailyRewardClaimed ? 'bg-gray-100' : 'bg-gradient-to-r from-yellow-400 to-orange-400'}`}>
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm font-bold">🔥 Streak: {streak} hari</div>
            <div className="text-xs text-gray-600">{nextRewardIn}</div>
          </div>
          <button
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="text-center bg-yellow-100 rounded-xl p-3">
          ...
          <div className="text-sm">Total Bintang</div>
        </div>
        <div className="text-center bg-green-100 rounded-xl p-3">
          <div className="text-3xl">🎮</div>
          <div className="text-2xl font-bold">{stats?.total_puzzles_completed || 0}</div>
          <div className="text-sm">Game Selesai</div>
        </div>
      </div>

      {/* Badges */}
      <div className="mb-4">
        <p className="font-bold mb-2">🏅 Lencana:</p>
        <div className="flex flex-wrap gap-2">
          {(stats?.badges || []).map((badge, i) => (
            <span key={i} className="bg-purple-200 px-3 py-1 rounded-full text-sm flex items-center gap-1">
              <span>🏅</span> {badge}
            </span>
          ))}
          {(stats?.badges || []).length === 0 && (
            <span className="text-gray-500 text-sm">Belum ada lencana. Ayo main!</span>
          )}
        </div>
      </div>
      ...
      {/* Stickers */}
      <div className="mb-4">
        <p className="font-bold mb-2">📌 Stiker Koleksi ({stats?.stickers?.length || 0}/20):</p>
        <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">
          {(stats?.stickers || []).map((sticker, i) => (
            <span key={i} className="text-2xl hover:scale-125 transition cursor-pointer" title={`Stiker ${i+1}`}>
              {sticker}
            </span>
          ))}
          {(stats?.stickers || []).length === 0 && (
            <span className="text-gray-500 text-sm">Main setiap hari dapat stiker!</span>
          )}
        </div>
      </div>
      ...
      <div className="mb-4 pt-4 border-t">
        <p className="font-bold mb-2">🔓 Tema Terbuka:</p>
        <div className="flex flex-wrap gap-2">
          {unlockedThemes.map(theme => (
            <span key={theme} className="bg-blue-200 px-3 py-1 rounded-full text-sm capitalize">
      ...
      <button
        ...
        className="w-full bg-gradient-to-r from-green-400 to-teal-400 text-white py-2 rounded-full font-bold mb-2 hover:scale-105 transition"
      >
        🛒 Toko Bintang ({stats?.total_stars || 0} ⭐)
      </button>

      {showShop && (
        <div className="mt-3 p-3 bg-green-50 rounded-xl max-h-80 overflow-y-auto">
          <p className="font-bold text-center mb-2">Belanja dengan Bintang:</p>
      ...
      {/* Message Toast */}
      {message && (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-full text-sm z-50 animate-bounce">
          {message}
        </div>
      )}
    </div>
