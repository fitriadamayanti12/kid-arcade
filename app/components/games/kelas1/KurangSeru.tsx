// app/components/games/kelas1/KurangSeru.tsx
'use client';
...
export default function KurangSeru({ onComplete }: Props) {
  ...
  const generate = () => {
    ...
    while (wrongs.size < 3) {
      const w = ans + (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 4) + 1);
      if (w !== ans && w >= 0 && w <= 20) wrongs.add(w);
    }
  ...
  if (step === 'menu') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <p style={{ fontSize: '16px', color: theme.textSecondary, marginBottom: '24px' }}>Latih pengurangan 1-20 dengan cepat! ⚡</p>
    ...
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <div style={{ background: theme.bgCard, borderRadius: '24px', padding: '30px', boxShadow: theme.shadow, marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px' }}>
          <span style={{ fontSize: '48px', fontWeight: '900', color: '#3b82f6' }}>{q.a}</span>
          <span style={{ fontSize: '36px', fontWeight: '900', color: theme.heading }}>-</span>
          <span style={{ fontSize: '48px', fontWeight: '900', color: '#ef4444' }}>{q.b}</span>
          <span style={{ fontSize: '36px', fontWeight: '900', color: theme.heading }}>=</span>
          <span style={{ fontSize: '48px', fontWeight: '900', color: '#f59e0b' }}>?</span>
        </div>
      </div>
