// app/components/games/kelas1/TambahAsyik.tsx
'use client';
...
export default function TambahAsyik({ onComplete }: Props) {
  ...
  const generate = () => {
    ...
    while (wrongs.size < 3) {
      ...
      if (w !== ans && w > 0 && w <= 30) wrongs.add(w);
    }
  ...
  if (step === 'menu') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <p style={{ fontSize: '16px', color: theme.textSecondary, marginBottom: '24px' }}>Latih penjumlahan 1-20 dengan cepat! ⚡</p>
      <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', fontSize: '40px', marginBottom: '24px' }}>
        <span>15 + 12 = 27</span>
      </div>
      <button onClick={start} style={{ padding: '14px 36px', fontSize: '18px', fontWeight: '700', borderRadius: '999px', border: 'none', background: '#3b82f6', color: '#fff', cursor: 'pointer', boxShadow: '0 4px 15px rgba(59,130,246,0.3)' }}>🎮 Mulai!</button>
    </div>
  );
    ...
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <div style={{ background: theme.bgCard, borderRadius: '24px', padding: '30px', boxShadow: theme.shadow, marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', marginBottom: '12px' }}>
          <div style={{ background: '#eff6ff', borderRadius: '16px', padding: '20px', minWidth: '80px' }}>
            <span style={{ fontSize: '40px', fontWeight: '900', color: '#3b82f6' }}>{q.a}</span>
          </div>
          <span style={{ fontSize: '36px', fontWeight: '900', color: theme.heading }}>+</span>
          <div style={{ background: '#fef2f2', borderRadius: '16px', padding: '20px', minWidth: '80px' }}>
            <span style={{ fontSize: '40px', fontWeight: '900', color: '#ef4444' }}>{q.b}</span>
          </div>
          <span style={{ fontSize: '36px', fontWeight: '900', color: theme.heading }}>=</span>
          <span style={{ fontSize: '40px', fontWeight: '900', color: '#f59e0b' }}>?</span>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <div style={{ background: theme.bgCard, borderRadius: '20px', padding: '20px', margin: '20px 0' }}>
        ...
        <p style={{ fontSize: '14px', color: theme.textMuted }}>Streak terbaik: {streak}x 🔥</p>
