// app/components/games/kelas2/KurangCepat.tsx
'use client';
...
export default function KurangCepat({ onComplete }: Props) {
  ...
  const generate = () => {
    ...
    const b = Math.floor(Math.random() * (a - 10)) + 5; // 5 to a-10
    const ans = a - b;
    const wrongs = new Set<number>();
    while (wrongs.size < 3) {
      const w = ans + (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 8) + 1);
      if (w !== ans && w >= 0 && w <= 80) wrongs.add(w);
    }
  ...
  if (step === 'menu') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '30px', fontWeight: '800', color: theme.heading }}>Kurang Cepat!</h2>
      <p style={{ fontSize: '16px', color: theme.textSecondary, marginBottom: '24px' }}>Latih pengurangan sampai 100! 🎯</p>
      <button onClick={start} style={{ padding: '14px 36px', fontSize: '18px', fontWeight: '700', borderRadius: '999px', border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer', boxShadow: '0 4px 15px rgba(239,68,68,0.3)' }}>⚡ Mulai Cepat!</button>
    </div>
  );
  ...
  if (step === 'play') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <div style={{ width: '100%', height: '6px', background: theme.border, borderRadius: '3px', marginBottom: '20px' }}>
        <div style={{ width: `${(total / 15) * 100}%`, height: '100%', background: 'linear-gradient(90deg, #ef4444, #f87171)', borderRadius: '3px', transition: 'width 0.3s' }} />
      </div>
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '28px', fontWeight: '800', color: theme.heading }}>Pengurangan Jago!</h2>
