// app/components/games/kelas4/KPKFPB.tsx
'use client';
...
export default function KPKFPB({ onComplete }: Props) {
  const theme = useThemeStyles();
  const [step, setStep] = useState<'menu' | 'play' | 'complete'>('menu');
  const [mode, setMode] = useState<'kpk' | 'fpb'>('kpk');
  const [q, setQ] = useState({ a: 0, b: 0, answer: 0, opts: [] as number[] });
  ...
  if (step === 'menu') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '30px', fontWeight: '800', color: theme.heading }}>KPK & FPB!</h2>
      <p style={{ fontSize: '16px', color: theme.textSecondary, marginBottom: '24px' }}>Kelipatan Persekutuan Terkecil & Faktor Persekutuan Terbesar 🔢</p>
  ...
  if (step === 'play') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '20px', textAlign: 'center', minHeight: '400px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
        ...
        <span style={{ fontSize: '14px', fontWeight: '700', color: mode === 'kpk' ? '#8b5cf6' : '#f59e0b' }}>
          {mode === 'kpk' ? 'KPK' : 'FPB'}
        </span>
      </div>

      <div style={{ background: theme.bgCard, borderRadius: '24px', padding: '30px', boxShadow: theme.shadow, marginBottom: '20px' }}>
        <h3 style={{ fontSize: '20px', color: theme.heading, marginBottom: '16px' }}>
          {mode === 'kpk' ? 'KPK' : 'FPB'} dari:
        </h3>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px' }}>
          <span style={{ fontSize: '42px', fontWeight: '900', color: '#8b5cf6' }}>{q.a}</span>
          <span style={{ fontSize: '24px', color: theme.textMuted }}>dan</span>
          <span style={{ fontSize: '42px', fontWeight: '900', color: '#f59e0b' }}>{q.b}</span>
        </div>
        <p style={{ fontSize: '24px', fontWeight: '900', color: theme.heading, marginTop: '16px' }}>= ?</p>
      </div>
      ...
      {selected !== null && (
        <div style={{ marginTop: '12px', padding: '14px', borderRadius: '12px', background: correct ? '#d1fae5' : '#fee2e2', color: correct ? '#065f46' : '#991b1b', fontWeight: '700', fontSize: '18px', animation: 'pop 0.3s ease-out' }}>
          {correct ? `🎉 ${mode.toUpperCase()} = ${q.answer}` : `❌ ${mode.toUpperCase()} = ${q.answer}`}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '28px', fontWeight: '800', color: theme.heading }}>Master KPK FPB!</h2>
