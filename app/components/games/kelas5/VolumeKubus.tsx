// app/components/games/kelas5/VolumeKubus.tsx
'use client';
...
export default function VolumeKubus({ onComplete }: Props) {
  ...
  const generate = () => {
    const types = [
      () => { const s = Math.floor(Math.random() * 10) + 2; return { text: `Kubus dengan sisi ${s} cm. Volume = ?`, answer: s * s * s }; },
      () => { const p = Math.floor(Math.random() * 10) + 2; const l = Math.floor(Math.random() * 8) + 2; const t = Math.floor(Math.random() * 8) + 2; return { text: `Balok ${p}×${l}×${t} cm. Volume = ?`, answer: p * l * t }; },
    ];
  ...
  if (step === 'menu') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '30px', fontWeight: '800', color: theme.heading }}>Volume!</h2>
      <p style={{ fontSize: '16px', color: theme.textSecondary, marginBottom: '24px' }}>Hitung volume kubus dan balok! 🧊</p>
  ...
  if (step === 'play') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <div style={{ background: theme.bgCard, borderRadius: '24px', padding: '30px', boxShadow: theme.shadow, marginBottom: '20px' }}>
        ...
        <p style={{ fontSize: '14px', color: theme.textMuted, marginTop: '8px' }}>cm³</p>
      </div>
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '28px', fontWeight: '800', color: theme.heading }}>Jago Volume!</h2>
