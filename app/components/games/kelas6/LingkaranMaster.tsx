// app/components/games/kelas6/LingkaranMaster.tsx
'use client';
...
export default function LingkaranMaster({ onComplete }: Props) {
  ...
  const generate = () => {
    const pi = 22/7;
    const types = [
      () => { const r = Math.floor(Math.random() * 14) + 7; return { text: `Lingkaran jari-jari ${r} cm (π=22/7). Luas = ?`, answer: Math.round(pi * r * r) }; },
      () => { const r = Math.floor(Math.random() * 14) + 7; return { text: `Lingkaran jari-jari ${r} cm (π=22/7). Keliling = ?`, answer: Math.round(2 * pi * r) }; },
      () => { const d = Math.floor(Math.random() * 14) + 7; return { text: `Lingkaran diameter ${d} cm (π=3,14). Keliling = ?`, answer: Math.round(3.14 * d) }; },
    ];
  ...
  if (step === 'menu') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '30px', fontWeight: '800', color: theme.heading }}>Lingkaran Master!</h2>
      <p style={{ fontSize: '16px', color: theme.textSecondary, marginBottom: '24px' }}>Kuasai luas & keliling lingkaran! 🎯</p>
  ...
  if (step === 'play') return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <div style={{ background: theme.bgCard, borderRadius: '24px', padding: '30px', boxShadow: theme.shadow, marginBottom: '20px' }}>
        <svg viewBox="0 0 200 200" style={{ width: '150px', height: '150px', margin: '0 auto' }}>
          <circle cx="100" cy="100" r="70" fill="none" stroke="#06b6d4" strokeWidth="3"/>
          <line x1="100" y1="100" x2="170" y2="100" stroke="#ef4444" strokeWidth="2" strokeDasharray="4"/>
          <text x="135" y="90" textAnchor="middle" fontSize="14" fontWeight="bold" fill={theme.text}>r</text>
        </svg>
        <h3 style={{ fontSize: '24px', fontWeight: '900', color: theme.heading, marginTop: '12px' }}>{q.text}</h3>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', maxWidth: '300px', margin: '0 auto' }}>
        {q.opts.map((opt, i) => (
          ...
          >{opt} cm{opt === q.answer && q.text.includes('Luas') ? '²' : ''}</button>
        ))}
      </div>
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', padding: '30px 20px', textAlign: 'center', minHeight: '400px' }}>
      ...
      <h2 style={{ fontSize: '28px', fontWeight: '800', color: theme.heading }}>Master Lingkaran!</h2>
